...
(function() {
  $(document).on('page:change', function() {
    ...
    $(document).on('notification', function(e, data) {
      return $.get("/partials/notification/" + data.notification.id, function(response) {
        $notificationButton.addClass('alert');
        return $notifications.prepend(response);
      });
    });
    return $(document).on('click', 'body, #notifications-button, #notification-list', function(e) {
      var $this;
      e.stopPropagation();
      $this = $(this);
      if ($this.is('#notifications-button')) {
        ...
        return $notifications.fadeIn(500);
      } else if ($this.is('body')) {
        return $notifications.fadeOut();
      }
    });
  });
...
(function() {
  var _escapeRegexp, _highlightWord;
  ...
  _highlightWord = function(pattern, question) {
    var $content, regExp;
    regExp = new RegExp("" + (_escapeRegexp(pattern)), 'i');
    $content = question.thumb.find('.content a');
    return $content.html(question.content.replace(regExp, '<span class="highlight-word">$&</span>'));
  };

  $(document).on('page:change', function() {
    var $queries, $searchBar, autofillbar, questionMemo, updateAutoFill, updateQueryList, _searchMemo, _searchQuestionHashes;
    $queries = $('#question-queries');
    $searchBar = $('#new_question #question_content');
    autofillbar = new autoFillBar('#new_question #question_content');
    questionMemo = {};
    _searchQuestionHashes = function(hashes, pattern) {
      return _.uniq(_.select(hashes, function(hash) {
        return pattern.test(hash.content);
      }));
    };
    _searchMemo = function(query) {
      var regExp;
      regExp = new RegExp(".*" + (_escapeRegexp(query)) + ".*", 'i');
      return _searchQuestionHashes(_.flatten(_.pluck(_.values(questionMemo), 'questions')), regExp);
    };
    updateQueryList = function(query) {
      if (questionMemo[query].count > 0) {
        $queries.html('');
        _.each(questionMemo[query].questions, function(question) {
          if (!(question.thumb instanceof $)) {
            question.thumb = $(question.thumb);
          }
          _highlightWord(query, question);
          return $queries.append(question.thumb);
        });
        return $queries.stop(true, true).slideDown('fast');
      } else {
        return $queries.stop(true, true).fadeOut('fast');
      }
    };
    updateAutoFill = function(query) {
      var firstQuestion, regExp, _ref;
      if ($queries.html() !== '') {
        regExp = new RegExp("^" + (_escapeRegexp(query)), 'i');
        if (regExp.test(firstQuestion = (_ref = _.first(questionMemo[query].questions)) != null ? _ref.content : void 0)) {
          return autofillbar.fill(query + firstQuestion.slice(query.length, firstQuestion.length));
        }
      }
    };
    $('#new_question #question_content').on('keyup', function(e) {
      var aSimilarFailure, memoResults, query;
      autofillbar.fill('');
      query = $(this).val();
      if (!query) {
      ...
      aSimilarFailure = _.any(_.keys(questionMemo), function(pastQuery) {
        var regExp;
        regExp = new RegExp("^" + (_escapeRegexp(pastQuery)), 'i');
        return regExp.test(query) && questionMemo[pastQuery].count < 1;
      });
      if (!aSimilarFailure) {
        if (questionMemo[query]) {
          updateQueryList(query);
          return updateAutoFill(query);
          ...
          return $queries.stop(true, true).slideUp('fast', function() {
            return $queries.html('');
          });
        } else {
    ...
    return $(document).on('click focusin focusout', 'body, #question-queries, #new_question #question_content', function(e) {
      var $this;
      e.stopPropagation();
      $this = $(this);
      if ($queries.html() !== '' && e.type === 'focusin' && $this.is('#new_question #question_content')) {
        return $queries.stop(true, true).slideDown('fast');
      } else if (e.type === 'click' && $this.is('body')) {
        return $queries.stop(true, true).slideUp('fast');
      }
    });
  });
